...
void PrintGameObjectSamples(
    const std::vector<er2::FindObjectsOfTypeAllResult> &gameObjects,
    std::size_t maxCount) {
  const std::size_t sampleCount =
      gameObjects.size() < maxCount ? gameObjects.size() : maxCount;
  for (std::size_t index = 0; index < sampleCount; ++index) {
    ...
    std::printf("[smoke] GameObject[%zu] native=0x%llX instanceId=%u name=%s\n",
                index, static_cast<unsigned long long>(item.native),
                item.instanceId, gotName ? name.c_str() : "<read-failed>");
  }
}

bool ValidateNamedGameObjects(const std::vector<std::uintptr_t> &objects,
  ...
  for (std::size_t index = 0; index < objects.size(); ++index) {
    ...
    std::printf("[smoke] NamedObject[%zu] native=0x%llX readName=%s\n", index,
...
bool ValidateUnityObjectCachedPtrOffset(
    const er2::FindObjectsOfTypeAllResult &sample) {
  if (!er2::SupportsDynamicFieldOffsets()) {
    std::printf("[smoke] DynamicOffset skipped: runtime=%s\n",
                er2::ToString(er2::Runtime()));
    return true;
  }

  std::uint32_t cachedPtrOffset = 0;
  if (!er2::TryGetFieldOffset("UnityEngine.Object", "m_CachedPtr",
                              cachedPtrOffset)) {
    std::printf(
        "[smoke] TryGetFieldOffset(UnityEngine.Object::m_CachedPtr) failed\n");
    return false;
  }

  std::uintptr_t managedObject = 0;
  if (!er2::ReadPtr(sample.native + er2::Off().unity_object_managed_ptr,
                    managedObject) ||
      managedObject == 0) {
    std::printf("[smoke] Failed to read managed object for native=0x%llX\n",
                static_cast<unsigned long long>(sample.native));
    return false;
  }

  std::uintptr_t cachedPtr = 0;
  if (!er2::ReadPtr(managedObject + static_cast<std::uintptr_t>(cachedPtrOffset),
                    cachedPtr) ||
      cachedPtr == 0) {
    std::printf("[smoke] Failed to read m_CachedPtr managed=0x%llX "
                "offset=0x%X\n",
  ...
  std::printf("[smoke] DynamicOffset UnityEngine.Object::m_CachedPtr "
              "offset=0x%X metadataRegistration=0x%llX "
              "fieldOffsetsPtr=0x%llX managed=0x%llX cachedPtr=0x%llX "
              "native=0x%llX\n",
              cachedPtrOffset,
              static_cast<unsigned long long>(
                  er2::FieldOffsetsMetadataRegistrationVa()),
...
int main() {
  ...
  std::printf("[smoke] AutoInit ok pid=%u runtime=%s gom=0x%llX msid=0x%llX\n",
  ...
  const std::vector<er2::FindObjectsOfTypeAllResult> gameObjects =
      er2::FindObjectsOfTypeAll("GameObject");
  ...
  const std::vector<std::uintptr_t> namedObjects =
      er2::GetGameObjectByName(targetGameObjectName);
  ...
  std::printf("[smoke] Overlap(%s, GameObject)=%zu\n", targetGameObjectName,
              overlapCount);
  if (overlapCount == 0) {
    std::printf("[smoke] %s results did not overlap with GameObject "
                "enumeration\n",
                targetGameObjectName);
    return 5;
  }
