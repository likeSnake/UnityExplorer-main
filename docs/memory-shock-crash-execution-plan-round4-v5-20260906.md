# 内存振闪退问题：第四轮发布执行计划与操作 AI 契约（round4-v5）
...
`round4-v2`、`round4-v3-draft` 和 `round4-v4` 均保留为历史评审材料，不能作为执行授权。本文件替代它们的执行语义；历史记录不改写。

本发布文件在任何批准回执签发后视为不可变。后续修订必须新建文件并递增 `planVersion`；不得覆盖本文件，否则已有回执全部失效并只能按 STOP 处理。
...
stateDiagram-v2
    ...
    G0_READ_ONLY --> CLOSED: G0 收单或资格不通过
    E0_APPROVABLE --> E0_RUNNING: PM 回执精确匹配
    E0_RUNNING --> CLOSED: 归档结果或 STOPPED
    CLOSED --> E0_APPROVABLE: G0 PASS 已收单；新 PID E0 preflight/候选已冻结且 PM 接受
    CLOSED --> G2_READ_ONLY: 仅 E0 PASS 已收单且 PM 单独授权
    G2_READ_ONLY --> CLOSED: G2 PASS 或 BLOCKED 收单
    CLOSED --> E1_APPROVABLE: G2 PASS 已收单、E1 输入冻结且 PM 接受候选
    E1_APPROVABLE --> E1_RUNNING: PM 回执精确匹配
    E1_RUNNING --> CLOSED: 任一 E1 结果
    CLOSED --> D_RUNNING: 第9节解除条件全部满足、前序收单 hash 验证且 PM 回执精确匹配
...
写实验候选/执行（E0、E1、D1a-D5）建立独立目录和账本：
...
G0 是只读资格阶段，不得伪造写实验文件。G0 目录只允许以下文件：

```text
action-script.json            # 冻结的三次正常动作脚本
precondition-manifest.json    # 进程身份、链快照、400B 基线和页/主机证据索引
timeline.jsonl                # 只读采样、动作、观察和退出事件
protection.txt                # PE section、VAD/PTE 或 UNKNOWN
host-telemetry.txt            # 主机证据配置及原始查询结果
result.md                     # 观察事实、资格判定（PASS/INCONCLUSIVE）
evidence-manifest.json        # G0 文件清单，不含自身
g0-ledger.md                  # G0 账本和收单请求
```
G0 的 `evidence-manifest.json` 使用第 5.3 节相同的编码和 hash 字段，按上列七个交付文件的字典序列出完整文件 hash，不含自身。G0 不得创建 `allowed-writes.json`、`rollback-plan.json`、`runtime-effects.json`、`outcome-rules.json`、`approval-receipt.md` 或任何目标写入清单；这些文件只能在 G0 被 PM 收单为 `PASS` 后、针对不同新 PID 的 E0 候选中创建。
...
`STOPPED` 是执行状态，此时 `experimentOutcome=NOT_EVALUATED`；它不是实验结果。允许配对为：`NOT_STARTED+NOT_EVALUATED`、`STARTED+NOT_EVALUATED`（尚未进入批准动作）、`STOPPED+NOT_EVALUATED`、`FINISHED+PASS|CRASH|EXIT_UNKNOWN|INCONCLUSIVE`。一旦已进入批准动作观察窗口后目标退出，即使原因无法分类，也必须使用 `FINISHED+EXIT_UNKNOWN` 或 `FINISHED+INCONCLUSIVE`，不能改写为 STOPPED。`FINISHED` 仅说明交付物已归档。`CRASH` 仅表示已观察到目标异常退出；没有绑定的原始退出码、PID/`processStartUtc`、目标映像证据和时间窗口时，一律使用 `EXIT_UNKNOWN` 或 `INCONCLUSIVE`。次级主机证据只能证明“退出与实验时段相关”，不能证明 AV、写保护 fault、主动退出或具体指令位置，也不能满足任何要求 faulting IP 的 Gate。

发现 STOP 条件后，立即停止尚未执行的正向实验写入。只有存在已确认成功的正向写入且目标仍存活时，才能依照批准的 `lastConfirmedForwardOrdinal -> rollbackOrdinals` 矩阵执行回滚；回滚顺序为已适用范围内的 `control=0` 并 readback -> 无输入空闲帧 -> vtable 恢复 `originalRet` 并 readback -> 恢复本实验 patch 和完整 T4 block 并 readback。不得对未执行步骤写入“回滚值”。任一回滚的 expected-before 或 readback 失败即停止一切进一步写入，标记 `rollbackDisposition: COMPROMISED_RESTART_REQUIRED`。进程已经退出时，账本必须写明 `rollbackDisposition: PROCESS_EXITED_RESTART_REQUIRED`；不得对已退出进程写入，也不得在新进程沿用旧地址或“补回滚”。

## 5. F1：冻结计划、独立批准回执与证据收单

本节替换 v3 的 F1。原则是先冻结完整计划，再由项目经理签发回执；`approvalId` 不属于计划记录，因此不存在 hash 自引用，也不存在批准后由操作 AI 补全写入清单的空间。所有路径均为相对于实验 `evidenceRoot` 的 ASCII、`/` 分隔路径；字段语法统一为 `fieldName: <单行 ASCII canonical value><LF>`。
...
编码必须是 UTF-8 without BOM；行尾必须是 LF；最后一行也必须有 LF；字段名只能出现一次；不得插入空行、注释、多行值或重复字段。每行实际格式为 `fieldName: value<LF>`，冒号后仅一个 ASCII 空格；值只能是 ASCII 可打印字符，路径使用 `/`，时间为 UTC RFC3339，地址为 `0x` 加小写十六进制；值不得包含 LF、CR、制表符或前后空格。固定字段顺序如下：
...
`ledgerPath` 指向 UTF-8/LF 的单一账本文件；其字节 0 必须是 `PLAN_RECORD_BEGIN<LF>`，并紧接本节的 `planRecordSha256` trailer。`ledgerPrefix` 精确定义为字节 0 至该 trailer 终止 LF；`ledgerPrefixSha256` 是该完整前缀的独立 SHA-256，不能与不含 trailer 的 `planRecordSha256` 混用。批准回执只存在于单独的 `approval-receipt.md`，不复制进账本前缀；结果只可在 manifest 冻结后追加。

### 5.2 外部冻结输入

下面六个文件是计划内容的一部分，必须在批准前生成，并采用 UTF-8 without BOM、LF、无尾随空格、规范化 JSON（对象键按规定顺序、数组按 `ordinal` 排序）。每个文件的 hash 写入计划段：

| 文件 | 必须包含的内容 |
|---|---|
| `precondition-manifest.json` | PID、`processStartUtc`、`generation`、目标映像路径/hash、GA/EXE 基址、module build、游戏状态、三次链快照、`t4StartVa`、`t4Length=400`、`t4BaselineSha256`、`nonCrossPage`、B/S/temp/vtable/`originalRet`/CrossFade，以及每个 DMA 写入范围（T4、patch、vtable/control）的页 VA、映射身份、PE section、跨页检查和保护采集时间 |
...
| `rollback-plan.json` | `control=0`、空闲帧、vtable 恢复、patch/block 恢复的顺序；每项 expected-before/readback；`lastConfirmedForwardOrdinal -> rollbackOrdinals` 矩阵；进程退出时的 `RESTART_REQUIRED` 处置 |
...
`allowed-writes.json` 必须包含完整的、可执行的写入值，而不是“写 block”“写 control”之类的摘要。每一候选只包含该实验真正需要的 DMA 写入与回滚，不得携带其他实验变量。所有条目必须附 `phase: FORWARD|ROLLBACK` 与唯一 `ordinal`。`control=0` 的 T4 基线验证是只读 `read-check`，不得列为 FORWARD 写入；如需存活时回滚，则单独列为 ROLLBACK 写入。E0 候选必须逐项包含初始 T4 block、vtable、E0 patch、`control=1`，以及适用的 `control=0`、vtable 恢复、patch 恢复和 block 恢复；E1 或 D 候选必须按其独立计划列出自己的完整集合。操作 AI 不得依照回执文本之外的推断补充写入。
...
`approvalReceiptSha256` 是回执 hash 范围的 SHA-256，作为 `approvalReceiptSha256: <64 位小写十六进制>` 的边界外唯一 trailer 写入；trailer 不在自身 hash 范围内。操作 AI 执行前必须独立复算计划、六个冻结输入和回执 hash，并逐项比较计划、回执与实时只读 preflight。`publishedPlanPath` 必须是本文件，`publishedPlanSha256` 必须等于其实际文件 hash；候选中的 `nextGateOnPass` 与 `nextGateOnNonPass` 必须均为 `CLOSED`。任何不等、缺文件、过期、PID 重用、`expectedBefore` 不匹配或额外写入需求都必须停止尚未执行的正向写入。若已有正向 ordinal 成功，只能执行 rollback matrix 中由该 ordinal 触发、且 expected-before 已重新核验的回滚写入；不得对未执行步骤回滚。批准只对这一份进程实例、一个实验 ID 和这个全量写入数组有效。

批准前的候选 preflight 若触发 STOP，只能写入 `preflight-stop.md`、只读 `timeline.jsonl` 和已取得的证据；它不是已批准写实验，不创建 `approval-receipt.md`、`evidence-manifest.json` 或 `RESULT_RECORD`，并以 `executionState=STOPPED`、`experimentOutcome=NOT_EVALUATED` 交给项目经理。不得为满足目录格式而补造回执或空写入清单。

最终归档顺序固定为：先完成并冻结六个计划输入、`timeline.jsonl`、`before-after.json`、`protection.txt`、`host-telemetry.txt`、`approval-receipt.md`、仅含观察事实的 `result.md` 与账本的计划/批准部分；`result.md` 在此阶段的最终字段必须是 `finalOutcome: PENDING_MANIFEST_VERIFICATION`。接着按下表生成 `evidence-manifest.json`，并在生成后禁止修改其覆盖文件：

`evidence-manifest.json` 使用 UTF-8 without BOM、LF、`schemaVersion: 1`，根目录为 `evidenceRoot`，条目按 `relativePath` 字典序升序；每项固定为 `relativePath`、`byteLength`、`sha256`（小写十六进制）。必须包含六个冻结输入、`timeline.jsonl`、`before-after.json`、`protection.txt`、`host-telemetry.txt`、`approval-receipt.md`、观察事实 `result.md`，以及账本从字节 0 至 `planRecordSha256` trailer 终止 LF 的 `ledgerPrefix`（其 `relativePath` 为 `ledgerPath`，hash 对象为该前缀字节）。明确排除 manifest 自身、`RESULT_RECORD`/其 trailer、PM 收单回执。

计算 `evidenceManifestSha256` 后，最后只允许在账本追加结果段：
...
G0 只有在以下所有条件成立时才可判 `PASS`：三次链与 400B hash 稳定；每个地址的映射身份（当前 PID、VA、页与模块/匿名映射来源）可确定；三次冻结动作均完成 30 秒观察且目标存活；没有新增的目标异常关键事件；时间线和主机时间可对齐。G0 记录不全、地址不稳定、section 与已取得 VAD/PTE 结论冲突、映射身份不明、观察窗口退出或主机时间无法对齐时，结果为 `INCONCLUSIVE`。`VAD/PTE=UNKNOWN` 只表示保护属性未知，不等于映射身份未知；它不会证明或否定 E0，但会阻断任何依赖 CPU 可写 scratch 的 E1。

## 7. E0：无 CPU 运行时写入的对照实验

E0 是目前唯一可以在 G0 完整收单后申请批准的写实验。E0 必须运行于与 G0 不同的新 PID，并针对该新 PID 重做完整三次只读 preflight、400B baseline、映射身份和页证据；仅 `action-script.json` 可以逐字节复用 G0 版本。它仍包含 DMA 写入；“无运行时写入”仅指 shell 在游戏 CPU 执行期间不写入 codecave 或 scratch。
...
E0 的批准候选必须把下列实际操作完整编码进 `allowed-writes.json` 和 `action-script.json`，而不是把它们作为口头步骤：写入并回读 400B T4 block；vtable 写 `S` 并回读；以只读 `control=0` 检查验证一次 T4 基线；写入并回读 8B E0 patch；写 `control=1` 并回读；执行 3 次固定动作且每次观察 30 秒；随后按第 4 节回滚。每个写入在批准前已有精确地址、长度、写前值、数据、readback rule 和回滚字节。批准前还必须在 `precondition-manifest.json` 冻结、并由 hash 绑定的反汇编/可达性证据证明：从 E0 入口到公共恢复区的全部可达 CPU `store` 集为空；`runtime-effects.json` 的完整字节严格为 `[]`；B 页可映射且可执行。`VAD/PTE=UNKNOWN` 仅豁免对 B 页 CPU 可写性的判断，不豁免这些映射、执行与无 store 证明。
...
G2 是资格 Gate，不使用 `experimentOutcome`；它必须在 `g2-result.md` 中记录 `gateQualification: PASS | BLOCKED`。任一项缺失时，`gateQualification=BLOCKED`，E1 仍不可申请。G2 不授权任何试写。
...
以下任一情况立即停止尚未执行的正向目标写入：hash 或回执不匹配；写前值/readback 不匹配；地址、跳转位移或长度无法由冻结文件核算；跨页或地址所在映射身份不明；E1 或任一 D 实验所依赖的 CPU 页保护未知；链不稳定；动作时间无法对齐；日志相互矛盾；需要新 scratch、fallback RVA、shell 模板、NOP 或额外写入。已生效前序步骤只能按第 4 节的批准 rollback matrix 处理。观察窗口内目标退出但原因不能分类时，停止进一步写入并以 `FINISHED+EXIT_UNKNOWN` 或 `FINISHED+INCONCLUSIVE` 收单，不得改报 STOPPED。G0 中记录为 `VAD/PTE=UNKNOWN` 的 B 页只允许进入明确写有“无 CPU 运行时写入”的 E0 候选，不能作为 E1/D 实验的 CPU 写目标依据。若目标存活，执行适用的已批准回滚清单；若目标退出，记录 `PROCESS_EXITED_RESTART_REQUIRED`。
