# 内存振闪退问题：第四轮执行计划与操作 AI 约束（round4-v3 定稿）

日期：2026-09-06
文档角色：项目经理执行文档
适用对象：负责实际 MCP/DMA 实验的操作 AI
前置记录：`docs/memory-shock-crash-update-round3-20260906.md`
文档状态：**RELEASED**——本文件是唯一有效的 round4-v3 执行计划；`round4-v2` 和 `round4-v3-draft` 仅供审计对比。

## 修订记录（F1–F10）

| 编号 | 严重度 | 修订位置 | 修订内容摘要 |
|---|---|---|---|
| F1 | 高 | 强制操作协议 §5 不可变实验账本 | 以边界化、规范化的 `planRecordSha256` 将操作 AI 提交的精确计划绑定到项目经理批准 |
| F2 | 高 | Gate G1 判定 | 明确 E0 非 PASS（CRASH/EXIT_UNKNOWN）⇒ 所有写实验（含 E1）阻塞，仅允许 G0 级只读采集与主机取证 |
| F3 | 中 | Gate G4 表格 D2 | 确认 `EB 7F` 会跳过中段链；因原实验语义错误，D2 及后续有副作用的变体保持 `BLOCKED` |
...
| F10 | 低 | Gate G0 / Gate G5 | 固化 400B T4 block、360B shell、32B metadata 与已撤销 24B 捕获尾的边界 |
...
【v3新增 F8】approvalId 生命周期规则：每个 approvalId 绑定签发时的 `planVersion`；若 `planVersion` 变更，所有未回传最终状态的 approvalId 自动失效，必须由项目经理重新签发。历史账本不因版本变更而改写。
...
`currentGate=CLOSED` 时，没有任何隐式、历史或口头授权。操作 AI 必须先创建规范化计划记录、计算 hash 并提交 `PROPOSED`。项目经理核对后才可签发批准；未收到下列完整记录等同于 `DENIED`，且不得执行任何目标内存写入：
...
planDocumentSha256: <本定稿的 SHA-256>
planRecordSha256: <操作 AI 提交的精确计划记录 SHA-256>
targetIdentity: <PID, processStartUtc, generation, module build>
preconditionEvidenceSha256: <已审计证据清单 hash>
allowedWriteSetSha256: <有序完整 write 列表 hash，不得只写摘要>
actionScriptSha256: <固定动作脚本 hash>
stopOn: <本计划的停止条件引用>
```
...
操作 AI 的首条回执只能是 `PROPOSED` 或 `STOPPED`。`PROPOSED` 必须附账本路径、`planRecordSha256`、目标身份和完整 write 列表；项目经理批准后，操作 AI 才能回传 `STARTED` 并执行计划。交付状态只允许为 `STARTED` 或 `DELIVERED`；实验结果只能是 `PASS`、`CRASH`、`EXIT_UNKNOWN`、`STOPPED` 或 `INCONCLUSIVE`。任何需要重新解释地址、改字节、补做验证或重复实验的情况，均使该批准失效，必须由项目经理重新签发新的 `approvalId`。
