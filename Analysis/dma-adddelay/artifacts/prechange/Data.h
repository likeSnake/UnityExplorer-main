...
#include <array>
#include <atomic>
// [FIX] Ensure std namespace is available for SDK.h
using namespace std;
#include "SDK.h"
...
// [OPTIMIZATION] Mimalloc include
#include "libs/mimalloc/include/mimalloc.h"

// [OPTIMIZATION] 定义 DMA 专用向量容器
// 使用 mi_heap_stl_allocator 自动为每个 vector 实例创建一个独立的 Heap
// (物理内存池) 这样可以确保 g_GameData
// 等核心数据的内存是连续的、隔离的，不与其他杂乱数据混在一起。
template <typename T>
using DMA_Vector = std::vector<T, mi_heap_stl_allocator<T>>;

namespace Global {
inline struct {
  std::unique_ptr<CharacterManager> Character =
      std::make_unique<CharacterManager>();
  std::unique_ptr<UserDataManager> UserData =
      std::make_unique<UserDataManager>();
  std::unique_ptr<BuffManager> Buff =
      std::make_unique<BuffManager>();
...
inline struct {
  ...
  // 恢复物品状态：缓存第 3 动画层与其当前 PlayableState 指针。
  uintptr_t Cache_MedicineRuntimeLogicLayer_PTR = NULL;
  uintptr_t Cache_MedicinePlayableState_PTR = NULL;
  ...
  uintptr_t Cache_BuffHandler_PTR = NULL;
} Cache_LocalPlayer;
...
struct PlayerData {
  ...
  int O_ReactionParryType;
  int O_CollisionFlags = 0;
  float O_ActionDuration;
  double O_ActionStartTime = 0;
  float O_Direction;
...
inline constexpr size_t kLocalAnimationLayerCapacity = 16;
inline constexpr size_t kLocalRuntimePropertyCapacity = 512;

struct LocalAnimationLayerState {
  ...
  int tagHash = 0;
  float weight = 0.0f;
  float stateStartTimeOffset = 0.0f;
  float duration = 0.0f;
  float currentTime = 0.0f;
  float transitionDuration = 0.0f;
  float transitionPercent = 0.0f;
  bool currentIsLogicState = false;
  bool transitionFinished = false;
  std::array<wchar_t, 64> animatorName{};
};

struct LocalRuntimePropertyState {
  int dataId = 0;
  int index = -1;
  int value = 0;
};

// 仅供“打印角色状态”调试使用。固定容量避免两个高频线程共享动态容器。
struct LocalCharacterStateDebug {
  bool valid = false;
  uint64_t sampleId = 0;
  uintptr_t actorKitInteractive = 0;
  uintptr_t animPlayable = 0;

  int skillCastStatus = 0;
  int beControlledStatus = 0;
  int heroBattleStatus = 0;
  int comboId = 0;
  int currentStageType = 0;
  int stereoDodgeType = 0;
  int lastStateHash = 0;
  int currentUseItemSkinId = 0;
  int lastBeControlledStatus = 0;
  int lastBaseStateHash = 0;
  int uniqueSkillState = 0;
  int jiguansanState = 0;

  int interactiveTargetFid = 0;
  uintptr_t canRescueActor = 0;
  uintptr_t canStealActor = 0;
  int dyingExecuteActorFid = 0;
  int interactiveType = 0;
  int interactiveStatus = 0;
  int interactionType = 0;
  int rescueFid = 0;
  int nowShowInteractiveType = 0;
  bool isMainPlayer = false;

  int animationLayerCount = 0;
  std::array<LocalAnimationLayerState, kLocalAnimationLayerCapacity>
      animationLayers{};
  int runtimePropertyCount = 0;
  int runtimePropertyValueCount = 0;
  std::array<LocalRuntimePropertyState, kLocalRuntimePropertyCapacity>
      runtimeProperties{};
};
...
struct LocalPlayer_Data1 {
  ...
  int MyTeam;
  int MyCurEnergy;
  int MyDodgeEnergyCost;
  int MyCurshield;
  bool MyIsUsingMedicine = false;
  int MyCurkuorong;
  ...
  int myNowBagWeapon;
  int myNowBagsoul;
  std::vector<int> myBuffTidList;
  std::vector<int> mySoulItemTidList;
  std::vector<int> myBagItemTidList;
  std::unordered_map<int, int> myBagItemCountByTid;
  int _showExtraEvent;
  bool IsSpectator;
  bool IsFireUltActive;
  double worldPtrGlobalTime;
  LocalCharacterStateDebug characterState;
...
extern int 跳出方式;
extern std::shared_mutex g_LocalInventoryMutex;
extern std::shared_mutex g_LocalCharacterStateMutex;
extern long 双刀上次出蓄力时间;
extern long 长剑上次出蓄力时间;
extern long 上次Shiftsh时间;
extern long 敌人上次闪避时间;
extern long 我上次闪避时间;
extern long 我上次主动闪避时间;
extern long 我上次切刀间;
extern long 我上次主动技能时间;
extern int 闪避后处理;
extern float 闪避后出刀间隔;
extern int 平A方式;
...
extern bool 振刀模式键按下;
extern bool 闪避模式键按下;
extern bool g_EnableAutoGameTheory;
extern bool 调试模式;
extern bool 活化闪避;
extern bool 英雄技能释放;
extern bool MimallocStats;
extern int 等待后摇时间;
// 双缓存数据结构
class DoubleBuffer {
private:
  DMA_Vector<PlayerData> buffers[2];        // 双缓冲存储
  std::atomic<int> read_index{0};           // 当前读缓冲区索引
  std::atomic<uint64_t> data_version{0};    // 每次交换递增的数据版本
  std::atomic<ULONGLONG> last_swap_tick{0}; // 最近一次数据交换时间
  std::atomic<uintptr_t> focused_actor_model{0}; // 连招期间固定目标
  std::atomic<ULONGLONG> focused_actor_expire_tick{0};
...
public:
  ...
  // 获取当前可读缓冲区（无锁）
  const DMA_Vector<PlayerData> &GetReadBuffer() const noexcept {
    return buffers[GetCurrentIndex()];
  }

  // 获取当前可写缓冲区（无锁）
  DMA_Vector<PlayerData> &GetWriteBuffer() noexcept {
  ...
  void SwapBuffers() {
    ...
    last_swap_tick.store(GetTickCount64(), std::memory_order_release);
    cv.notify_all();
  ...
  ULONGLONG GetDataAgeMs() const noexcept {
    const ULONGLONG tick = last_swap_tick.load(std::memory_order_acquire);
    return tick == 0 ? 0 : GetTickCount64() - tick;
  }

  uint64_t GetDataVersion() const noexcept {
    return data_version.load(std::memory_order_acquire);
  }

  uintptr_t GetActiveFocusedActorModel() const noexcept {
    const uintptr_t actor =
        focused_actor_model.load(std::memory_order_acquire);
    const ULONGLONG expire =
        focused_actor_expire_tick.load(std::memory_order_acquire);
    if (actor == 0 || expire < GetTickCount64()) {
      return 0;
    }
    return actor;
  }
  ...
  bool WaitForData(const std::chrono::duration<Rep, Period> &timeout) {
    ...
    return cv.wait_for(lock, timeout,
                       [this] { return !buffers[GetCurrentIndex()].empty(); });
  }

  // 等待指定消费者尚未处理过的新版本，避免 buffer 非空后的 1ms 轮询。
  template <typename Rep, typename Period>
  bool WaitForNewData(uint64_t last_seen_version,
                      const std::chrono::duration<Rep, Period> &timeout) {
    std::unique_lock<std::mutex> lock(swap_mutex);
    return cv.wait_for(lock, timeout, [this, last_seen_version] {
      return data_version.load(std::memory_order_acquire) !=
             last_seen_version;
    });
  }
  ...
  // 获取最近的玩家
  std::optional<PlayerData> GetNearestPlayer() const { return nearest_player; }

  void FocusPlayerTarget(const PlayerData &player,
                         ULONGLONG duration_ms = 3000) {
    const uintptr_t actor = player.O_ActorModel;
    if (actor == 0) {
      return;
    }

    std::lock_guard<std::mutex> lock(swap_mutex);
    focused_actor_model.store(actor, std::memory_order_release);
    focused_actor_expire_tick.store(GetTickCount64() + duration_ms,
                                    std::memory_order_release);
    nearest_player = player;
    FatPlayerData = player;
...
private:
  ...
  void UpdateNearestPlayer() {
    ...
    const uintptr_t focused_actor =
        focused_actor_model.load(std::memory_order_acquire);
    const ULONGLONG focused_expire =
        focused_actor_expire_tick.load(std::memory_order_acquire);
    const bool focus_active =
        focused_actor != 0 && focused_expire >= now;

    // 连招触发后优先沿用触发时的目标，避免每帧最近目标切换。
    if (focus_active) {
      for (const auto &player : current_buffer) {
        if (!player.IsSoul && player.O_ActorModel == focused_actor) {
          nearest_player = player;
          FatPlayerData = player;
          FatPlayerData.IsHaveData = true;
          return;
    ...
    } else if (focused_actor != 0) {
      focused_actor_model.store(0, std::memory_order_release);
      focused_actor_expire_tick.store(0, std::memory_order_release);
    }
...
extern std::atomic<int> NetState;
} // namespace Net
namespace B_Pro {
...
extern std::atomic<bool> DproState;
extern char IP[0XFF];
...
extern bool 横刀左蓄振刀;
extern bool 只振视野范围内;
extern int MemoryShockMode; // 0=当前逻辑, 1=第二套逻辑
extern bool 内存振变态模式;
extern bool 动画加速;
extern float 动画加速倍率;
extern float 振刀全局延迟修正;
extern int 振刀按键;
extern int 切拳按键;
extern int 大招按键;
extern int 滞空按键;
...
extern bool MedicineOnlyShockDodgeCloseEnemy; // 打药时只振/闪贴脸敌人
...
inline DMA_Vector<ObjData> g_GameData;      // [OPTIMIZATION] 使用独立堆
inline DMA_Vector<PlayerData> g_PlayerData; // [OPTIMIZATION] 使用独立堆
inline std::atomic<uint64_t> g_localplayer_reads_per_second{0};
inline std::atomic<ULONGLONG> g_local_combat_update_tick{0};
inline std::atomic<ULONGLONG> g_local_action_update_tick{0};
inline std::atomic<ULONGLONG> g_local_blue_update_tick{0};
inline std::atomic<uint64_t> g_localplayer_last_loop_us{0};
inline std::atomic<uint64_t> g_enemy_total_count{0};
inline std::atomic<uint64_t> g_shock_candidate_count{0};
inline std::atomic<uint64_t> g_enemy_combat_read_count{0};
inline std::atomic<uint64_t> g_enemy_combat_read_distance_x10{0};
inline DMA_Vector<ItemInfoData> 全_物品数据;        // [OPTIMIZATION] 使用独立堆
inline DMA_Vector<InteractiveInfoData> 全_金堆数据; // [OPTIMIZATION] 使用独立堆
inline DMA_Vector<ShenHuoPuzzleInfoData> 全_神火雕像数据; // 神火广场雕像目标状态
inline DMA_Vector<Vec2> 全_骨骼数据;                // [OPTIMIZATION] 使用独立堆
extern LocalPlayer_Data1 LocalPlayer_Data;

extern DoubleBuffer data_buffers;
extern std::mutex g_InteractiveMutex;

int GetLocalBagItemCountById(int itemId);
...
inline map<uint32_t, InteraData> InteractiveTable = {
    ...
    {4004261, {"[落物堆]-绿59", Color::竹篁绿, 1}},
    {4002450, {"[落物堆]-绿60", Color::竹篁绿, 1}},
    {4004264, {"[落物堆]-绿61", Color::竹篁绿, 1}},
    {4002987, {"[落物堆]-绿62", Color::竹篁绿, 1}},
    {4002453, {"[落物堆]-绿63", Color::竹篁绿, 1}},
    ...
    {4004262, {"[落物堆]-蓝45", Color::星蓝, 2}},
    {4002451, {"[落物堆]-蓝45", Color::星蓝, 2}},
    {4004265, {"[落物堆]-蓝47", Color::星蓝, 2}},
    {4002985, {"[落物堆]-蓝48", Color::星蓝, 2}},
    {4002454, {"[落物堆]-蓝49", Color::星蓝, 2}},
    ...
    {4004263, {"[落物堆]-金58", Color::向日葵黄, 3}},
    {4002452, {"[落物堆]-金58", Color::向日葵黄, 3}},
    {4004266, {"[落物堆]-金59", Color::向日葵黄, 3}},
    {4002986, {"[落物堆]-金59", Color::向日葵黄, 3}},
    {4002455, {"[落物堆]-金60", Color::向日葵黄, 3}},

    {3010171, {"金刚果", Color::向日葵黄, 4}},
...
extern float 乌真大招4M;
extern float 乌真大招8M;
extern float 乌真大招12M;
extern float 乌真大招12M外;

extern float 叶修大招矛;
extern float 叶修大招镰;
extern float 叶修炮形态4M;
extern float 叶修炮形态8M;
extern float 叶修炮形态12M;
extern float 叶修炮形态12M外;
