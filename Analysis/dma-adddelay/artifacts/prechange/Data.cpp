...
std::shared_mutex g_LocalInventoryMutex;
std::shared_mutex g_LocalCharacterStateMutex;

int 跳出方式 = 0;
int 平A方式 = 0;
bool MimallocStats = false;
long 双刀上次出蓄力时间;
long 长剑上次出蓄力时间;
long 上次Shiftsh时间;
long 敌人上次闪避时间;
...
int GetLocalBagItemCountById(int itemId) {
  ...
  std::shared_lock lock(g_LocalInventoryMutex);
  const auto it = LocalPlayer_Data.myBagItemCountByTid.find(itemId);
  if (it == LocalPlayer_Data.myBagItemCountByTid.end()) {
    return 0;
  }
  return it->second;
}
...
float userTextSize = 18;
} // namespace ESP
namespace Kmbox {
namespace Net {
char IP[0XFF], Port[0XFF], mac[0XFF];
std::atomic<int> NetState{5};
} // namespace Net
namespace B_Pro {
...
std::atomic<bool> DproState{false};
...
int 振刀模式热键 = VK_XBUTTON2;
int 闪避模式热键 = VK_XBUTTON1;
...
bool MedicineOnlyShockDodgeCloseEnemy = false; // 打药时只振/闪贴脸敌人
...
float 狐狸V1 = 15.f;      // 0-20M
float 狐狸V2 = 7.5f;      // 0-20M
float 乌真V = 12.5f;      // 0-20M
float 火男F = 5.f;        // 0-5M
float 岳山F = 7.5f;       // 0-5M
