...
public:
   ...
   * @param memMap if true, will dump the memory map to a file	& make the DMA
   * use it.
   * @return true if successful, false if not.
   */
  bool Init(std::string process_name, bool memMap = true, bool debug = false);

  /*This part here is things related to the process information such as Base
   * daddy, Size ect.*/
   ...
   */
  uintptr_t GetExportTableAddress(std::string import, std::string process,
                                  std::string module);
   ...
   */
  uintptr_t GetImportTableAddress(std::string import, std::string process,
                                  std::string module);

  /**
   * \brief This fixes the CR3 fuckery that EAC does.
   * It fixes it by iterating over all DTB's that exist within your system and
   * looks for specific ones that nolonger have a PID assigned to them, aka
   * their pid is 0 it then puts it in a vector to later try each possible DTB
   * to find the DTB of the process. NOTE: Using FixCR3 requires you to have
   * symsrv.dll, dbghelp.dll and info.db
   */
  bool FixCr3();

  /**
   * \brief Dumps the process memory at address (requires to be a valid PE
   * Header) to the path
   ...
   * @return the value read from the process
   */
  template <typename T> T Read(void *address, int pid) {
    T buffer{};
    memset(&buffer, 0, sizeof(T));
    Read(reinterpret_cast<uint64_t>(address), reinterpret_cast<void *>(&buffer),
         sizeof(T), pid);
  ...
  template <typename T> T Read(uint64_t address, int pid) {
  ...
  T ReadChain(uint64_t base, const std::vector<uint64_t> &offsets) {
    ...
    for (int i = 1; i < offsets.size(); i++)
      result = Read<T>(result + offsets.at(i));
    return result;
  }

  /**
   * \brief Create a scatter handle, this is used for scatter read/write
   * requests
  ...
  void AddScatterReadRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address,
                             T *buffer) {
    AddScatterReadRequest(handle, address, reinterpret_cast<void *>(buffer),
                          sizeof(T));
  }

  void AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t address,
                              void *buffer, size_t size);

  /**
   * \brief Executes all prepared scatter requests, note if you created a
   * scatter handle with a pid you'll need to specify the pid in the execute
   * function. so we can clear the scatters from the handle.
   * \param handle
   * \param pid
   */
