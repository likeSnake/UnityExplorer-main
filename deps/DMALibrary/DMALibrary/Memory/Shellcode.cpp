...
std::vector<std::string> blacklist = {
    ...
    "wow64cpu.dll", "ntoskrnl.exe",   "win32kbase.sys"};

uint64_t c_shellcode::find_codecave(size_t function_size,
  ...
  if (!VMMDLL_ProcessGetInformation(mem.vHandle, pid, &process_info,
                                    &process_info_size)) {
  ...
  DWORD cSections = 0;
  if (!VMMDLL_ProcessGetSectionsU(mem.vHandle, pid,
                                  const_cast<LPSTR>(module.c_str()), NULL, 0,
                                  &cSections) ||
      !cSections) {
  ...
  const PIMAGE_SECTION_HEADER pSections = static_cast<PIMAGE_SECTION_HEADER>(
      LocalAlloc(LMEM_ZEROINIT, cSections * sizeof(IMAGE_SECTION_HEADER)));
  if (!pSections ||
      !VMMDLL_ProcessGetSectionsU(mem.vHandle, pid,
                                  const_cast<LPSTR>(module.c_str()), pSections,
                                  cSections, &cSections) ||
      !cSections) {
    ...
    if (!codecave &&
        ((pSections[i].Characteristics &
          (IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_MEM_WRITE |
           IMAGE_SCN_MEM_READ))) &&
        ((pSections[i].Misc.VirtualSize & 0xfff) < (0x1000 - function_size))) {
      codecave =
          VMMDLL_ProcessGetModuleBaseU(mem.vHandle, pid,
                                       const_cast<LPSTR>(module.c_str())) +
          ((pSections[i].VirtualAddress + pSections[i].Misc.VirtualSize)) +
          0x10;
      if (!codecave)
        break;
    }
...
std::vector<uint64_t>
c_shellcode::find_all_codecave(size_t function_size,
                               const std::string &process_name) {
  std::vector<uint64_t> codecaves = {};
  std::vector<std::string> module_list = mem.GetModuleList(process_name);
  for (size_t i = 0; i < module_list.size(); i++) {
    if (std::find(blacklist.begin(), blacklist.end(), module_list[i]) !=
        blacklist.end())
      continue;
...
bool c_shellcode::call_function(void *hook, void *function,
                                const std::string &process_name) {
  int pid = mem.GetPidFromName(process_name);
  ...
  // Set jump address
  *reinterpret_cast<uint64_t *>(jmp_bytes + 6) =
      reinterpret_cast<uintptr_t>(hook);

  auto orig_bytes = std::unique_ptr<uint8_t[]>(new uint8_t[sizeof(jmp_bytes)]);
  if (!mem.Read(reinterpret_cast<uintptr_t>(function), orig_bytes.get(),
                sizeof(jmp_bytes), pid))
    return 0;

  if (!VMMDLL_MemWrite(mem.vHandle, pid, reinterpret_cast<uintptr_t>(function),
                       jmp_bytes, sizeof(jmp_bytes))) {
  ...
  // Restore function
  if (!VMMDLL_MemWrite(mem.vHandle, pid, reinterpret_cast<uintptr_t>(function),
                       orig_bytes.get(), sizeof(jmp_bytes))) {
