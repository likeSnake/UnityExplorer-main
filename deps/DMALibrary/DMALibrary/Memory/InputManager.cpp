  ...
  if (Winver > 22000) {
    ...
    for (size_t i = 0; i < pids.size(); i++) {
      ...
      if (!VMMDLL_Map_GetModuleFromNameW(
              mem.vHandle, pid, const_cast<LPWSTR>(L"win32ksgd.sys"),
              &win32k_module_info, VMMDLL_MODULE_FLAG_NORMAL)) {
        if (!VMMDLL_Map_GetModuleFromNameW(
                mem.vHandle, pid, const_cast<LPWSTR>(L"win32k.sys"),
                &win32k_module_info, VMMDLL_MODULE_FLAG_NORMAL)) {
      ...
      if (!g_session_ptr) {
        // win32k
        g_session_ptr = mem.FindSignature("48 8B 05 ? ? ? ? FF C9", win32k_base,
                                          win32k_base + win32k_size, pid);
        if (!g_session_ptr) {
      ...
      for (int i = 0; i < 4; i++) {
        user_session_state = mem.Read<uintptr_t>(
            mem.Read<uintptr_t>(
                mem.Read<uintptr_t>(g_session_global_slots, pid) + 8 * i, pid),
            pid);
      ...
      if (!VMMDLL_Map_GetModuleFromNameW(
              mem.vHandle, pid, const_cast<LPWSTR>(L"win32kbase.sys"),
      ...
      uintptr_t ptr = mem.FindSignature("48 8D 90 ? ? ? ? E8 ? ? ? ? 0F 57 C0",
                                        win32kbase_base,
    ...
    bool result =
        VMMDLL_Map_GetEATU(mem.vHandle,
                           mem.GetPidFromName("winlogon.exe") |
    ...
    if (gafAsyncKeyStateExport < 0x7FFFFFFFFFFF) {
      ...
      if (!VMMDLL_PdbSymbolAddress(mem.vHandle, str,
...
void c_keys::UpdateKeys() {
  ...
  VMMDLL_MemReadEx(
      mem.vHandle, this->win_logon_pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY,
      gafAsyncKeyStateExport, reinterpret_cast<PBYTE>(&state_bitmap), 64, NULL,
      VMMDLL_FLAG_NOCACHE);
