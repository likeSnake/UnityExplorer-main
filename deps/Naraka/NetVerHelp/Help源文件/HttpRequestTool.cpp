...
#define SECURE_REQUEST   NORMAL_REQUEST | INTERNET_FLAG_SECURE | INTERNET_FLAG_IGNORE_CERT_CN_INVALID
	...
	string CHttpRequestTool::ExecutePostRequest(CString strUrl, CString	m_strPostData, list<string> head, int timeOut)
	{
		...
		try
		{
			...
			{
				wchar_t* OptionText;
				OptionText = m_strPostData.AllocSysString();
				int nLen = WideCharToMultiByte(CP_UTF8, 0, OptionText, -1, NULL, 0, NULL, NULL);
				char *pBuf = new char[nLen + 1];
				if (NULL != pBuf)
				{
					WideCharToMultiByte(CP_UTF8, 0, OptionText, -1, pBuf, nLen, NULL, NULL);
	...
	{
		wchar_t* OptionText;
		OptionText = strText.AllocSysString();
