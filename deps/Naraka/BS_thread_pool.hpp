...
#ifdef BS_THREAD_POOL_ENABLE_PRIORITY
/**
 * @brief 用于指示任务优先级的类型。定义为宽度（至少）为 16 位的整数.
 */
...
namespace this_thread {
    /**
     * @brief 由 `BS::this thread::get index()` 返回的类型，如果该线程属于 `BS::thread pool`，则可以选择性地包含线程的索引。否则，它将不包含任何值。
     */
    using optional_index = std::optional<size_t>;

    /**
     * @brief `BS::this thread::get pool()` 返回的类型，如果该线程属于 `BS::thread pool`，则可以选择性地包含指向拥有该线程的池的指针。否则，它将不包含任何值。
     */
...
public:
    ...
    /**
     * @brief 检查存储在这个 `multi future` 中的所有 Future 是否有效。
    *
    * @return 如果所有 Future 都有效，则返回 `true`；如果至少有一个 Future 无效，则返回 `false`。
     */
