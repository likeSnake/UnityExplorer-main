...
struct Context {
  ...
  std::shared_ptr<IMemoryAccessor> memory; // Added Memory Accessor Interface
...
// [Log Callback]
inline std::function<void(const std::string &)> g_logCallback;
...
inline void AutoInitLog(const char *fmt, ...) {
  ...
  Log(std::string("[AUTO_INIT] ") + buffer);
  std::fprintf(stderr, "[AUTO_INIT] %s\n", buffer);
  std::fflush(stderr);
}
...
inline bool GetContextModuleInfo(std::uint32_t pid, const wchar_t *moduleName,
                                 ModuleInfo &out) {
  out = ModuleInfo{};
  if (EnsureContextBackend() &&
      g_ctx.backend->GetModuleInfo(pid, moduleName, out) && out.base) {
    return true;
  }
...
inline void ResetContext() {
  ...
  const bool processOwnedByBackend =
      backend && backend->GetProcessHandle() == g_ctx.process;

  if (backend) {
    backend->Reset();
  }

  if (g_ctx.process && !processOwnedByBackend) {
  ...
  // memory shared_ptr reset automatically
}

inline void SetContextBackend(ContextBackendPtr backend) {
  const bool processOwnedByBackend =
      g_ctx.backend && g_ctx.backend->GetProcessHandle() == g_ctx.process;
  ...
  if (g_ctx.backend) {
    ...
    if (accessor) {
      g_ctx.memory = std::shared_ptr<IMemoryAccessor>(
          g_ctx.backend, const_cast<IMemoryAccessor *>(accessor));
    }
  }
}
...
inline bool InitSettings(std::uint32_t pid, ManagedBackend runtime) {
  ...
  if (!g_ctx.process && !g_ctx.memory) {
    ResetContext();
    return false;
  }

  // Default to WinAPI, can be overridden later or manually set
  if (!g_ctx.memory && g_ctx.process) {
    g_ctx.memory = std::make_shared<WinApiMemoryAccessor>(g_ctx.process);
  }
...
inline bool InitBase(std::uint64_t gomSlotRva,
                     std::uint64_t msIdToPointerSlotRva) {
  if (!g_ctx.process || !g_ctx.unityPlayer.base) {
    return false;
  }

  g_ctx.gomGlobalSlotRva = gomSlotRva;
  g_ctx.gomGlobalSlotVa =
      g_ctx.unityPlayer.base + static_cast<std::uintptr_t>(gomSlotRva);

  g_ctx.msIdToPointerSlotRva = msIdToPointerSlotRva;
  g_ctx.msIdToPointerSlotVa = g_ctx.unityPlayer.base +
                              static_cast<std::uintptr_t>(msIdToPointerSlotRva);
...
inline bool AutoInit() {
  ...
  if (pids.empty()) {
    ...
    AutoInitLog("fallback_find_pids_with_module(UnityPlayer.dll) count=%zu",
                pids.size());
  }
  ...
  for (const std::uint32_t pid : pids) {
    ...
    const ManagedBackend runtime =
        isIl2Cpp ? ManagedBackend::Il2Cpp : ManagedBackend::Mono;
    AutoInitLog("pid=%u runtime_candidate=%s", pid,
    ...
    // Use the interface
    const IMemoryAccessor &mem = *g_ctx.memory;

    const std::uint64_t tGomBegin = AutoInitNowMs();
    std::uint64_t gomSlotRva = 0;
    if (!FindGomGlobalSlotRvaByScan(mem, g_ctx.unityPlayer.base, g_ctx.gomOff,
                                    gomSlotRva)) {
      AutoInitLog("pid=%u gom_scan_failed ms=%llu", pid,
                  static_cast<unsigned long long>(AutoInitNowMs() -
                                                  tGomBegin));
      ResetContext();
      continue;
    }
    AutoInitLog("pid=%u gom_scan_ok gom_rva=0x%llX ms=%llu", pid,
    ...
    if (FindMsIdToPointerSlotVaByScan(mem, g_ctx.unityPlayer, g_ctx.gomOff,
      ...
      g_ctx.msIdToPointerSlotRva =
          static_cast<std::uint64_t>(msIdSlotVa - g_ctx.unityPlayer.base);
      AutoInitLog("pid=%u msid_scan_ok slot_va=0x%llX ms=%llu", pid,
                  static_cast<unsigned long long>(msIdSlotVa),
                  static_cast<unsigned long long>(AutoInitNowMs() -
                                                  tMsidBegin));
    } else {
      AutoInitLog("pid=%u msid_scan_failed ms=%llu", pid,
                  static_cast<unsigned long long>(AutoInitNowMs() -
                                                  tMsidBegin));
    }

    AutoInitLog("success pid=%u total_pid_ms=%llu total_ms=%llu", pid,
...
inline bool IsInited() {
  return g_ctx.pid != 0 && g_ctx.unityPlayer.base != 0 &&
         g_ctx.memory != nullptr;
}

inline std::uint32_t Pid() { return g_ctx.pid; }

inline ManagedBackend Runtime() { return g_ctx.runtime; }

inline std::uintptr_t UnityPlayerBase() { return g_ctx.unityPlayer.base; }

inline std::uintptr_t GomGlobalSlotVa() { return g_ctx.gomGlobalSlotVa; }

inline std::uintptr_t MsIdToPointerSlotVa() {
  return g_ctx.msIdToPointerSlotVa;
}

inline const Offsets &Off() { return g_ctx.off; }

inline const GomOffsets &GomOff() { return g_ctx.gomOff; }

inline const CameraOffsets &CamOff() { return g_ctx.camOff; }

inline const TransformOffsets &TransformOff() { return g_ctx.transformOff; }

inline IMemoryAccessor &Mem() {
  if (g_ctx.memory) {
    return *g_ctx.memory;
  }

  return const_cast<IMemoryAccessor &>(GetNullMemoryAccessor());
}
...
template <typename T> inline bool ReadValue(std::uintptr_t address, T &out) {
  if (!IsInited()) {
    out = T{};
    return false;
  }
