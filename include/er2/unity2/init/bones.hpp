...
inline std::vector<BoneTransformAllItem>
GetBoneTransformAll(std::uintptr_t rootGameObjectNative, int maxDepth = 10,
                    int maxNodes = 4096) {
  ...
  while (!stack.empty()) {
    ...
    stack.pop_back();
    if (cur.depth > maxDepth || !IsCanonicalUserPtr(cur.tr) ||
        visited.count(cur.tr))
      continue;
    visited.insert(cur.tr);
    std::uintptr_t curGo = 0;
    if (er2::GetNativeComponentGameObject(Mem(), cur.tr, g_ctx.gomOff, curGo) &&
        curGo) {
      BoneTransformAllItem item;
      item.transform = cur.tr;
      TransformHierarchyState st;
      std::int32_t idx = 0;
      if (er2::ReadTransformHierarchyState(Mem(), cur.tr, g_ctx.transformOff,
                                           st, idx))
        item.index = idx;
      std::string name;
      if (er2::ReadNativeGameObjectName(Mem(), curGo, g_ctx.gomOff, name))
        item.boneName = std::move(name);
      out.push_back(std::move(item));
      if ((int)out.size() >= maxNodes)
        break;
    }
    std::uintptr_t childrenPtr = 0;
    std::uint32_t childrenCount = 0;
    if (!er2::ReadPtr(Mem(), cur.tr + kTransformChildrenPtr, childrenPtr) ||
        !childrenPtr)
      continue;
    if (!er2::ReadValue(Mem(), cur.tr + kTransformChildrenCount,
                        childrenCount) ||
        childrenCount == 0 || childrenCount >= 100)
      continue;
    for (std::uint32_t i = 0; i < childrenCount; ++i) {
      std::uintptr_t child = 0;
      if (er2::ReadPtr(Mem(), childrenPtr + i * sizeof(std::uintptr_t),
                       child) &&
          child)
