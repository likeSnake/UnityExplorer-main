...
inline void ResetRuntimeState() { g_fieldOffsetState = RuntimeState{}; }

inline std::string BuildShortTypeName(const std::string &fullName,
...
inline std::string ResolveTypeKind(
  ...
  const auto parentIt =
      byvalToFullName.find(static_cast<std::uint32_t>(typeDef.parentIndex));
...
inline void AddAliasCandidate(
    const std::string &alias, const std::string &fullName,
    std::unordered_map<std::string, std::string> &aliasToFullName,
...
inline bool BuildRuntimeState(RuntimeState &out) {
  ...
  MetadataHint hint;
  if (!BuildMetadataHintTScore(mem, gameAssembly->base, Pid(), L"",
  ...
  if (!ExportMetadataByScore(mem, gameAssembly->base, 0x200000u, 8192, 15.0,
  ...
  std::uintptr_t fieldOffsetsPtr = 0;
  if (!DumpSdk6GetMetadataRegistrationTypes(mem, hint.metadataRegistration,
  ...
  const std::uint32_t fieldDefCount =
      header.fieldsSize /
      static_cast<std::uint32_t>(sizeof(DumpSdk6FieldDefRaw));
  ...
  for (std::uint32_t typeIndex = 0; typeIndex < typeDefCount; ++typeIndex) {
    ...
    const DumpSdk6TypeDefRaw *typeDef =
        reinterpret_cast<const DumpSdk6TypeDefRaw *>(metaBytes.data() +
                                                     typeBase);
    ...
    (void)ReadCStringFromMetadataBytes(metaBytes, header,
                                       typeDef->namespaceIndex, namespaceName);
    ...
    AddAliasCandidate(shortName, fullName, out.aliasToFullName,
                      ambiguousAliases);

    if (typeDef->parentIndex >= 0) {
      const auto parentIt = byvalToFullName.find(
          static_cast<std::uint32_t>(typeDef->parentIndex));
    ...
    if (!ReadPtr(mem, fieldOffsetsPtr +
                          static_cast<std::uintptr_t>(typeIndex) * 8u,
                 fieldOffsetArray) ||
        fieldOffsetArray == 0) {
      continue;
    }
    ...
    for (std::uint32_t fieldSlot = 0; fieldSlot < typeDef->fieldCount;
         ++fieldSlot) {
      const std::uint32_t fieldIndex =
          static_cast<std::uint32_t>(typeDef->fieldStart) + fieldSlot;
      ...
      (void)ReadCStringFromMetadataBytes(metaBytes, header, fieldDef->nameIndex,
                                         fieldName);
      if (fieldName.empty()) {
        continue;
      }
      ...
      if (!ReadValue(mem, fieldOffsetArray +
                              static_cast<std::uintptr_t>(fieldSlot) * 4u,
                     rawOffset)) {
        continue;
      }
...
inline bool TryResolveFullTypeName(const std::string &typeName,
  ...
  if (g_fieldOffsetState.fieldsByType.find(typeName) !=
      g_fieldOffsetState.fieldsByType.end()) {
    outFullName = typeName;
    return true;
  }

  if (g_fieldOffsetState.knownTypes.find(typeName) !=
      g_fieldOffsetState.knownTypes.end()) {
    outFullName = typeName;
    return true;
  }
...
inline bool TryGetFieldOffset(const std::string &typeName,
  ...
  while (!currentType.empty() && visitedTypes.insert(currentType).second) {
    const auto typeIt =
        detail_field_offset::g_fieldOffsetState.fieldsByType.find(currentType);
    ...
    const auto parentIt =
        detail_field_offset::g_fieldOffsetState.parentByType.find(currentType);
...
inline std::uint32_t GetFieldOffsetOr(const std::string &typeName,
                                      const std::string &fieldName,
