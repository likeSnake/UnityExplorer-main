...
public:
  ...
  virtual bool GetModuleInfo(std::uint32_t pid, const wchar_t *moduleName,
                             ModuleInfo &out) const = 0;
};

class WinApiContextBackend final : public IContextBackend {
public:
  ~WinApiContextBackend() override { Reset(); }
  ...
  HANDLE GetProcessHandle() const override { return m_process; }

  const IMemoryAccessor *GetMemoryAccessor() const override {
    return m_memoryAccessor.get();
  }

  bool GetModuleInfo(std::uint32_t pid, const wchar_t *moduleName,
                     ModuleInfo &out) const override {
