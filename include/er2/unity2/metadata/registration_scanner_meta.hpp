...
inline bool FindMetadataRegistration(const IMemoryAccessor &mem,
                                     ...
                                     std::size_t chunkSize, double maxSeconds,
  ...
  const std::vector<std::int64_t> typeDefCounts =
      InferTypeDefCounts(typeDefSize);
  ...
  for (const auto &sec : dataSecs) {
    ...
    const std::uintptr_t start =
        moduleBase + static_cast<std::uintptr_t>(sec.rva);
    ...
    while (offset < size && std::chrono::steady_clock::now() <= deadline) {
      ...
      const std::size_t toRead = remain > static_cast<std::uint32_t>(chunkSize)
                                     ? chunkSize
                                     : static_cast<std::size_t>(remain);
      if (!ReadChunk(mem, start + static_cast<std::uintptr_t>(offset), toRead,
                     buf)) {
      ...
      const std::size_t scanEnd =
          buf.size() - static_cast<std::size_t>(off.structSize);
      for (std::size_t i = 0; i <= scanEnd; i += 8) {
        const std::uintptr_t baseAddr = start +
                                        static_cast<std::uintptr_t>(offset) +
                                        static_cast<std::uintptr_t>(i);

        const std::int64_t fieldOffsetsCnt =
            I64At(buf, i + static_cast<std::size_t>(off.fieldOffsetsCount));
        const std::int64_t typeSizesCnt = I64At(
            buf, i + static_cast<std::size_t>(off.typeDefinitionsSizesCount));
        ...
        const std::int64_t typesCnt =
            I64At(buf, i + static_cast<std::size_t>(off.typesCount));
        if (typesCnt <= 0 || typesCnt > 300000) {
          continue;
        }

        const std::uint64_t typesPtr =
            U64At(buf, i + static_cast<std::size_t>(off.types));
        const std::uint64_t fieldOffsetsPtr =
            U64At(buf, i + static_cast<std::size_t>(off.fieldOffsets));
        const std::uint64_t typeSizesPtr =
            U64At(buf, i + static_cast<std::size_t>(off.typeDefinitionsSizes));
        ...
        for (int j = 0; j < 3; ++j) {
          std::uintptr_t p = 0;
          if (!ReadPtr(mem,
                       static_cast<std::uintptr_t>(typesPtr) +
                           static_cast<std::uintptr_t>(j) * 8u,
                       p)) {
            ok = false;
            break;
          }
        ...
        for (int j = 0; j < 3; ++j) {
          std::uintptr_t p = 0;
          if (!ReadPtr(mem,
                       static_cast<std::uintptr_t>(fieldOffsetsPtr) +
                           static_cast<std::uintptr_t>(j) * 8u,
                       p)) {
            ok = false;
            break;
...
inline bool FindMetadataRegistrationBySections(
    const IMemoryAccessor &mem, std::uintptr_t moduleBase,
    std::uint32_t moduleSize, const std::vector<ModuleSection> &sections,
    std::uintptr_t metaBase, std::size_t chunkSize, double maxSeconds,
    std::uintptr_t &outAddr) {
  outAddr = 0;
  if (moduleBase == 0 || moduleSize == 0 || metaBase == 0) {
    return false;
  ...
  std::vector<DiskSection> dsecs;
  dsecs.reserve(sections.size());
  for (const auto &s : sections) {
    DiskSection ds;
    std::memset(ds.name, 0, sizeof(ds.name));
    strncpy_s(ds.name, s.name.c_str(), 8);
    ds.rva = s.rva;
    ds.vsize = s.size;
    ds.rawPtr = 0;
    ds.rawSize = 0;
  ...
  std::vector<std::pair<std::uintptr_t, std::uintptr_t>> dataRanges;
  std::vector<DiskSection> dataSecs;
  BuildRanges(moduleBase, dsecs, execRanges, dataRanges, dataSecs);
  ...
  for (const auto &sec : dataSecs) {
    ...
    while (offset < size && std::chrono::steady_clock::now() <= deadline) {
      ...
      const std::size_t scanEnd =
          buf.size() < (std::size_t)off.structSize
              ? 0
              : buf.size() - static_cast<std::size_t>(off.structSize);
      for (std::size_t i = 0; i <= scanEnd; i += 8) {
        const std::uintptr_t baseAddr = start +
