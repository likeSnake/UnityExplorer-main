...
inline bool FindCodeRegistration(const IMemoryAccessor &mem,
                                 ...
                                 std::uintptr_t metaBase, std::size_t chunkSize,
                                 double maxSeconds, std::uintptr_t &outAddr) {
  outAddr = 0;
  ...
  const std::uintptr_t moduleEnd =
      moduleBase + static_cast<std::uintptr_t>(moduleSize);
  ...
  for (const auto &sec : dataSecs) {
    ...
    while (offset < size && std::chrono::steady_clock::now() <= deadline) {
      ...
      const std::size_t scanEnd =
          buf.size() - static_cast<std::size_t>(off.needBytes);
      for (std::size_t i = 0; i <= scanEnd; i += 8) {
        const std::uintptr_t baseAddr = start +
                                        static_cast<std::uintptr_t>(offset) +
                                        static_cast<std::uintptr_t>(i);

        const std::uint64_t cgCnt =
            U64At(buf, i + static_cast<std::size_t>(off.codeGenModulesCount));
        if (cgCnt != static_cast<std::uint64_t>(imageCount)) {
          continue;
        }

        const std::uint64_t cgPtr =
            U64At(buf, i + static_cast<std::size_t>(off.codeGenModules));
        if (cgPtr == 0) {
          continue;
        }
        if (!CheckCodeGenModulesArray(mem, moduleBase, moduleEnd,
                                      static_cast<std::uintptr_t>(cgPtr), 3)) {
          continue;
        }

        const std::uint64_t invCnt =
            U64At(buf, i + static_cast<std::size_t>(off.invokerPointersCount));
        const std::uint64_t invPtr =
            U64At(buf, i + static_cast<std::size_t>(off.invokerPointers));
        if (invCnt == 0 || invCnt > 10000000ull) {
          continue;
        }
        if (!CheckPointerArrayPointsIntoExec(
                mem, static_cast<std::uintptr_t>(invPtr), moduleBase, moduleEnd,
                execRanges, 3)) {
          continue;
        }
...
inline bool FindCodeRegistrationBySections(
  ...
  for (const auto &sec : dataSecs) {
    ...
    while (offset < size && std::chrono::steady_clock::now() <= deadline) {
      ...
      const std::size_t scanEnd =
          buf.size() < (std::size_t)off.needBytes
              ? 0
              : buf.size() - static_cast<std::size_t>(off.needBytes);
      for (std::size_t i = 0; i <= scanEnd; i += 8) {
        const std::uintptr_t baseAddr = start +
        ...
        const std::uint64_t cgPtr =
            U64At(buf, i + static_cast<std::size_t>(off.codeGenModules));
        if (cgPtr == 0 ||
            !InAny(static_cast<std::uintptr_t>(cgPtr), dataRanges)) {
          continue;
        }
