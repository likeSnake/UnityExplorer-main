...

{
  ...
  return s == ".data" || s == ".rdata" || s.find(".vmp") != std::string::npos ||
         s.find("_rdata") != std::string::npos;
}

inline std::uintptr_t
FindGomGlobalSlotByScanInternal(const IMemoryAccessor &mem,
                                std::uintptr_t moduleBase,
                                std::size_t chunkSize, const GomOffsets &off)

{
  ...
  const std::uintptr_t moduleEnd =
      moduleBase + static_cast<std::uintptr_t>(sizeOfImage);
  ...
  std::vector<std::uint8_t> pageBuf(0x1000); // 4KB Page Buffer

  for (const auto &sec : scanSections) {
    const std::uintptr_t secStart =
    ...
    // [Fix] DMA Robustness: Scan page-by-page (4KB)
    // Instead of reading the whole section (which fails if ANY page is swapped
    // out), we read page by page.
    for (std::size_t offset = 0; offset < sec.size; offset += 0x1000) {
      std::size_t bytesToRead = 0x1000;
      if (offset + bytesToRead > sec.size) {
        bytesToRead = sec.size - offset;
      }

      if (!mem.Read(secStart + offset, pageBuf.data(), bytesToRead)) {
        // If page is swapped out, verify it is just this page and continue
        continue;
      }

      const std::size_t limit = bytesToRead >= 8 ? (bytesToRead - 8) : 0;

      for (std::size_t off8 = 0; off8 <= limit; off8 += 8) {
        std::uint64_t ptr = 0;
        std::memcpy(&ptr, pageBuf.data() + off8, sizeof(ptr));
        ...
        const ManagerCandidateCheck r =
            CheckGameObjectManagerCandidateBlindScan(mem, manager, off);
        ...
        if (r.score > bestScore) {
          bestScore = r.score;
          bestGomGlobal = secStart + offset + static_cast<std::uintptr_t>(off8);
...
inline bool FindGomGlobalSlotRvaByScan(const IMemoryAccessor &mem,
...

{
  ...
  const std::uintptr_t slot =
      FindGomGlobalSlotByScanInternal(mem, unityPlayerBase, chunkSize, off);
